/* @flow weak */
"use strict";
var Resource = (module.exports = function(rawResource, headers, client) {
  rawResource = rawResource || {};
  return this._construct(rawResource, headers, client);
});
...
Resource.prototype._construct = function(rawResource, headers, client) {
...
Resource.prototype._checkIsResource = function(resource) {
  if (resource instanceof Resource) {
    if (!resource._base.id) {
      throw new Error(
        "Resource has not been created remotely, it can't be assigned to a relationship!"
      );
    }
    return;
  }
...
Resource.prototype._associateWith = function(resource) {
  ...
  Object.keys(relationships).forEach(function(relationName) {
    ...
    if (rawRelationMeta.relation === "foreign") {
      ...
      otherRelation = self;
      rawRelationData = resource._getRaw().relationships[rawRelationMeta.as]
        .data;
    }

    var match = !![]
      .concat(rawRelationData)
  ...
  if (getMeta(rawRelation).relation === "foreign") {
    throw new Error(
      "Relationship " +
        ...
        " on " +
        this._raw.type +
        " is a foreign relation and cannot be updated from here!"
    );
  }
...
Resource.prototype._addRelation = function(
  relationName,
  rawRelation,
  resource
) {
  ...
  if (!Resource._rawRelationHasMany(rawRelation)) {
    throw new Error(
      "Relationship " +
        ...
        " on " +
        this._raw.type +
        " is a not a MANY relationship and cannot be added to!"
    );
  }
...
Resource.prototype._removeRelation = function(
  relationName,
  rawRelation,
  resource
) {
...
Resource.prototype._setRelation = function(
  relationName,
  rawRelation,
  resource
) {
...
Resource.prototype.fetch = Promise.denodeify(function(
  relationName,
  options,
  callback
) {
  ...
  self._client._getRelated(this, relationName, options, function(
    err,
    newResources
  ) {
    ...
    self._raw.relationships[relationName].data = newResources.map(function(
      someResource
    ) {
...
Resource.prototype.sync = Promise.denodeify(function(callback) {
  ...
  target.call(self._client, self, function(err, rawResponse, rawResource) {
    ...
    self._construct(rawResource, self._headers, self._client);
