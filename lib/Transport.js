...
Transport.prototype._action = function(method, url, data, headers, callback) {
  ...
  if (headers) {
    Object.keys(headers).forEach(function(headerName) {
      someRequest.set(headerName, headers[headerName]);
    });
  }
  ...
  someRequest.end(function(err, payload) {
    ...
    return callback(
      ...
      payload.body,
      payload.body.data,
      payload.body.included
    );
  });
};

Transport.prototype.search = function(type, options, headers, callback) {
  var url = this._path + "/" + type;
  this._action("get", url, options, headers, callback);
};

Transport.prototype.find = function(type, id, options, headers, callback) {
...
Transport.prototype.fetch = function(resource, relation, options, callback) {
  ...
  this._action("get", url, options, resource._getHeaders(), callback);
};
...
Transport.prototype.update = function(resource, callback) {
  var url =
    this._path + "/" + resource._getBase().type + "/" + resource._getBase().id;
  this._action(
    "patch",
    url,
    {
...
Transport.prototype.delete = function(resource, callback) {
  var url =
    this._path + "/" + resource._getBase().type + "/" + resource._getBase().id;
  this._action("del", url, null, resource._getHeaders(), callback);
};
